...
class Recorder:
    def __init__(self,
                 ...
                 # if recording with multiple Recorders, set all but one to quiet_mode (no interface)
                 quiet_mode=False,
                 ):
    ...
    def video_writer(self):
        ...
        while self.camera.IsGrabbing() or not self.video_frames_queue.empty():
            ...
            if self.n_recorded_frames % 5 == 0 and not self.quiet_mode:
    ...
    def stop(self):
        ...
        self.camera.Close()  # TODO: why two camera.Close()?
