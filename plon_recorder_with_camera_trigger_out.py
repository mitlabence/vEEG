...
import pyaudio  # isntall with pip! Otherwise WASAPI not detected: https://stackoverflow.com/questions/47640188/pyaudio-cant-detect-sound-devices#:~:text=This%20issue%20generally%20arises%20when%20you%20are%20using,pyaudio%20Either%20of%20them%20will%20fix%20your%20issue.
...
class Recorder:
    ...
    def initialize_save_paths(self):
        ...
        self.video_save_path = self.save_path / f'video_{time.strftime("%Y%m%d_%H%M%S", self.start_time_struct)}'
        self.video_save_path.mkdir(parents=True, exist_ok=True)
        self.single_channel_audio_save_path = self.save_path / f'single_channel_audio_{time.strftime("%Y%m%d_%H%M%S", self.start_time_struct)}'
        self.single_channel_audio_save_path.mkdir(parents=True, exist_ok=True)
        self.log_save_path = self.save_path / f'log_{time.strftime("%Y%m%d_%H%M%S", self.start_time_struct)}.txt'

    def initialize_camera(self):
        self.plon_Tlfactory = plon.TlFactory.GetInstance()
        cameras = self.plon_Tlfactory.EnumerateDevices()
        self.camera = plon.InstantCamera(self.plon_Tlfactory.CreateDevice(cameras[self.camera_index]))
        self.camera.Open()
        self.camera.LineSelector.Value = "Line3"  # GPIO for acA1300-75gm
        self.camera.UserOutputSelector.Value = "UserOutput2"
    ...
    def send_trigger(self):  # sends negative, then positive spike on GPIO output of camera acA1300-75gm
        ...
        # See: https://docs.baslerweb.com/user-output-value with model above specified
        self.camera.UserOutputValueAll.Value = 0b100  # only change GPIO, should cause a negative spike
        self.camera.UserOutputValueAll.Value = 0b000  # should cause a positive spike

    def video_recorder(self):    
        
        while self.camera.IsGrabbing():
            grabResult = self.camera.RetrieveResult(5000, plon.TimeoutHandling_ThrowException)
            ...
            if self.camera.IsGrabbing() and grabResult.GrabSucceeded():
                ...
                if self.i_frame >= 2*self.camera_fps:
                    self.send_trigger()
                    self.i_frame = 0
                else:
                    self.i_frame += 1
    ...
    def video_writer(self):
        ...
        while self.camera.IsGrabbing() or not self.video_frames_queue.empty():
            if self.video_frames_queue.empty(): continue
    ...
    def initialize_microphone(self):
        ...
        host_api_info = self.pyaudio.get_host_api_info_by_type(pyaudio.paWASAPI)
        host_api_index = host_api_info.get('index')
        mic_num = host_api_info.get('deviceCount')
        mic_index = -1
        for i in range(mic_num):
            device_info = self.pyaudio.get_device_info_by_host_api_device_index(host_api_index, i)
            if device_info.get('name').lower().find(self.microphone_tag.lower()) != -1:
                mic_index = device_info.get('index')
                break
        if mic_index == -1:
            raise Exception(f"Microphone with tag {self.microphone_tag} not found")
        device_info = self.pyaudio.get_device_info_by_index(mic_index)
        self.microphone_sampling_rate = int(device_info.get('defaultSampleRate'))
        self.microphone_n_channels = device_info.get('maxInputChannels')
        self.microphone_high_latency = device_info.get('defaultHighInputLatency')
        ...
        self.audio_chunk_size = librosa.time_to_samples(chunk_duration, sr=self.microphone_sampling_rate)
        self.audio_sample_width = self.pyaudio.get_sample_size(pyaudio.paInt16)
        self.audio_format = self.pyaudio.get_format_from_width(self.audio_sample_width)
    ...
    def audio_writer(self):
        file_index = 1
        self.audio_output = wave.open(str((self.single_channel_audio_save_path / f'T{file_index:07d}.wav').absolute()), 'wb')
        ...
        while self.audio_stream.is_active() or not self.audio_chunks_queue.empty():
            if self.audio_chunks_queue.empty(): continue
    ...
    def initialize_audio_processing(self):
        ...
        self.freqs = librosa.fft_frequencies(sr=self.microphone_sampling_rate, n_fft=self.nfft)
        ...
        self.audio_cache = np.zeros((self.nfft_hop_ratio-1)*self.hop_length+self.audio_chunk_size)

    def audio_processor(self):
        def perform_stft():
            _, _, stft = signal.stft(self.audio_cache, nperseg=self.nfft, noverlap=self.audio_chunk_size - self.hop_length, nfft=self.nfft, axis=0, padded=False, boundary=None)
            self.stft_cache = np.roll(self.stft_cache, -self.nfft_hop_ratio, axis=0)
            np.copyto(self.stft_cache[-self.nfft_hop_ratio:, :], np.log10(np.abs(stft)).T)
        ...
        while self.audio_stream.is_active() or not self.processing_audio_chunks_queue.empty():
            ...
            self.audio_cache = np.roll(self.audio_cache, -self.audio_chunk_size)
    ...
    def logger(self):
        with open(self.log_save_path, 'w') as f:
            while True:
                if self.logger_queue.empty(): time.sleep(0.01); continue
                current_time, log = self.logger_queue.get()
                if log is None: break
                f.write(f'{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(current_time))}.{int((current_time - int(current_time)) * 1000):03d} {log}\n')
                f.flush()
    ...
    def interface(self):
        ...
        while self.camera.IsGrabbing() and self.audio_stream.is_active():
            if self.cv2_frames_queue.empty(): continue
            current_time, video_frame = self.cv2_frames_queue.get()
            video_frame = add_text(video_frame, "RECORDING" if current_time >= self.start_time else "NOT RECORDING")/255
            audio_stft_frame = self.stft_cache.copy()
            audio_stft_frame = cv2.resize(audio_stft_frame.T[self.freqs <= 125000, :][::-1, :], (self.video_width, self.video_height//4))
    ...
    def start(self):
        self.video_recorder_thread = threading.Thread(target=self.video_recorder)
        self.video_thread = threading.Thread(target=self.video_writer)
        self.audio_thread = threading.Thread(target=self.audio_writer)
        self.audio_processor_thread = threading.Thread(target=self.audio_processor)
